...
class Empresa {
	...
	function cadastrar($form) {
		...
		$result = $sqlGl->insertInto('empresa',$form);
	...
	function alterar($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->update('empresa')->set($form)->where('id', $form['id']);
	...
	function deletar($id) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->deleteFrom("empresa")->where("id",$id);
	...
	function listar($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("empresa")->where($atributos)->orderBy($orderBy);
	...
	function mostraNome($id) {
		...
		$aValores = $sqlGl -> from("empresa")->where('id',$id);
		$aValores = $aValores->fetch();
	...
	function publicar($id, $status) {
		global $sqlGl;
		$result = $sqlGl->update('empresa')->set('status',$status)->where('id', $id);
		$result = $result->execute(true);
	...
	function pegarListagensParaAlterarBack($fromPosition, $toPosition) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$aValores = $sqlGl -> from("empresa")->where('ordem <= :ordem1 && ordem >= :ordem2 ',array(':ordem1' => $fromPosition, ':ordem2' => $toPosition));
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGl;
		$result = $sqlGl->update('empresa')->set('ordem',$posicao)->where('id', $id);
		$result = $result->execute(true);
